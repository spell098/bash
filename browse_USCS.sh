...
for i in `seq 0 $[${#chr[@]}-1]`;do
	if [[ ${strand[$i]} = '+' ]]; then sequences[$i]=$(cat $path"Rattus_norvegicus.Rnor_5.0.dna.chromosome."${chr[$i]}".fa" | tail -n +2 | tr -d '\n' | cut -c $[${start[$i]}-$flank_down]-$[${start[$i]}+$flank_up])
	elif [[ ${strand[$i]} = '-' ]]; then sequences[$i]=$(cat $path"Rattus_norvegicus.Rnor_5.0.dna.chromosome."${chr[$i]}".fa" | tail -n +2 | tr -d '\n' | cut -c $[${stop[$i]}-$flank_up]-$[${stop[$i]}+$flank_down])
	fi
done;

##using curl
for i in `seq 0 $[${#chr[@]}-1]`;do
	if [[ ${strand[$i]} = '+' ]]; then sequence[$i]=$(curl http://genome.ucsc.edu/cgi-bin/das/hg19/dna?segment=${chr[$i]}:$[${start[$i]}-$flank_down],$[${start[$i]}+$flank_up];) 
	elif [[ ${strand[$i]} = '-' ]]; then sequence[$i]=$(curl http://genome.ucsc.edu/cgi-bin/das/hg19/dna?segment=${chr[$i]}:$[${stop[$i]}-$flank_up]-$[${stop[$i]}+$flank_down];) 
	fi
done;
